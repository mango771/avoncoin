...
// avoncoin: optional setting to unlock wallet for block minting only;
...
// avoncoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // avoncoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // avoncoin: sub-cent change is moved to fee
...
// avoncoin: create coin stake transaction
...
// avoncoin: check 'spent' consistency between wallet and txindex
// avoncoin: fix wallet spent state according to txindex
...
// avoncoin: disable transaction (only for coinstake)
